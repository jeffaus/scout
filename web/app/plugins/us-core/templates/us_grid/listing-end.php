...
if ( ! wp_doing_ajax() ) {
	...
	foreach ( us_get_filter_taxonomies( $filter_url_prefix, $us_grid_filter_params ) as $item_name => $item_value ) {
		...
		if ( $item_source === 'tax' ) {
			$_filter_taxonomies[ us_get_grid_filter_url_prefix() . '_' . $item_name ] = implode( ',', $item_value );
			...
			foreach ( us_arr_path( $query_args, 'meta_query', array() ) as $index => $meta_query ) {
				if ( is_array( $meta_query ) ) {
					if ( ! empty( $meta_query['key'] ) AND $meta_query['key'] === $item_name ) {
						unset( $query_args['meta_query'][ $index ] );
					} else {
						foreach ( $meta_query as $item_index => $item_meta_query ) {
							if ( ! empty( $item_meta_query['key'] ) AND $item_meta_query['key'] === $item_name ) {
								unset( $query_args['meta_query'][ $index ][ $item_index  ] );
							}
						}
					}
				}
